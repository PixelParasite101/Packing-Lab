...
const w1 = setupWorld(); w1.enableAdaptiveIterations(opts); w1.enableDeterminismMetrics(); runFrames(w1, 70, 0.9); const h1 = snapshot(w1);
const w2 = setupWorld(); w2.enableAdaptiveIterations(opts); w2.enableDeterminismMetrics(); runFrames(w2, 70, 0.9); const h2 = snapshot(w2);
const pass = h1 === h2;
console.log(`[AT24] Metrics Hash Determinism PASS=${pass} ${h1} vs ${h2}`);
